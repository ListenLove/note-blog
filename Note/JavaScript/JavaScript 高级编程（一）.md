...
3. 如果是对象特有的属性或者方法，不要存储到构造函数中。

4. `prototype`中的默认属性`constructor`，指向当前原型对象对应的“构造函数”。另外，每个“实例对象”都有一个默认属性`__proto__`指向创建他的那个构造函数的“原型对象”。
...
通过`__proto__`连接起来的链条就是**原型链**。对象的方法、属性查找就是通过这个链条进行查找，最后找到`Object`一级停止。
...
对象解构赋值的注意点：**等号左边的变量名称必须和对象的属性名称一致，才能解构出数据。**

### assign

`Object`的`assign`方法可以将第二个参数的对象和属性拷贝到第一参数的对象中，可以实现“伪深拷贝”。
